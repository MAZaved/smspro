...
class Admin extends CI_Controller
{
    ...
    function subject($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'do_update') {
            ...
            redirect(base_url() . 'inddex.php?admin/subject/' . $data['class_id'], 'refresh');
        } else if ($param1 == 'edit') {
...
//        $page_data['class_name'] = $this->db->get_where('class' , array('class_id' => $param1))->result_array();
//        foreach ($page_data['class_name'] as $cname)
//        {
//            $class_name=$cname['name'];
//        }
    ...
    function exam($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            $data['invisilator_id'] = $this->input->post('invisilator_id');
    ...
    /****MANAGE EXAM MARKS** test***/
    function marks($sort_by = 'student_id', $sort_order = 'desc', $code = 0, $page = 0, $rows = 15)
    {
        ...
        $this->Search_model->marksheet($post);
        ...
        $data['term'] = $term;
        $data['mark_data'] = $this->Search_model->get_marks($term, $sort_by, $sort_order, $rows, $page);


        $data['total'] = $this->Search_model->get_marks_count($term);

        $this->load->library('pagination');

        $config['base_url'] = site_url('admin/marks/' . $sort_by . '/' . $sort_order . '/' . $code . '/');
        ...
        $data['page_info'] = 'Exam marks';

        $data['page_name'] = 'marks_search';
        $data['page_title'] = get_phrase('manage_exam_marks');
    ...
    function save_marks()
    {
        ...
        foreach ($marks as $mark_id=>$mark){
            $marked['mark_id'] = $mark_id;
            $marked['mark_obtained']= $mark[0];

            $this->Search_model->marksheet($marked);
        }
        ...
        function tabulation_sheet($class_id = '', $exam_id = '')
        {
            ...
            if($class_id == 'search')
            {
                ...
                $page_data['tabulation'] = $this->Search_model->tabulation_search($student_id,$section_id);
                //echo 1;
                //print_r($page_data['tabulation']);
        ...
        function search_tabulation($param1)
        {
            if ($this->session->userdata('admin_login') != 1)
                redirect(base_url(), 'refresh');
            if($param1 == 'search')
            {
                ...
                $page_data['tabulation'] = $this->Search_model->tabulation_search($student_id,$section_id,$exam_id);
                //echo 1;
                //print_r($page_data['tabulation']);
                //redirect(base_url() . 'index.php?admin/tabulation_sheet/' . $page_data, 'refresh');
        ...
        /**********MANAGING CLASS ROUTINE******************/
        function test_class_routine($param1 = '', $param2 = '', $param3 = '')
        {
            if ($this->session->userdata('admin_login') != 1)
                redirect(base_url(), 'refresh');
            if ($param1 == 'create') {
            ...
            if ($param1 == 'do_update') {
                $data['class_id'] = $this->input->post('class_id');
                $data['subject_id'] = 1;//$this->input->post('subject_id');
                $data['time_start'] = $this->input->post('time_start');// + (12 * ($this->input->post('starting_ampm') - 1));
                $data['time_end'] = $this->input->post('time_end');// + (12 * ($this->input->post('ending_ampm') - 1));
                $data['time_start_min'] = $this->input->post('time_start_min');
                $data['time_end_min'] = $this->input->post('time_end_min');
            ...
            $page_data['test_class_routine'] = $this->db->get_where('class_routine', array('class_id' => $param1))->result_array();
            foreach ($page_data['test_class_routine'] as $row) {
                $sname = $row['subject_id'];
                //$page_data['subject_name'] = $this->db->query
                //$s=$this->db->query("SELECT name FROM  `subject` WHERE  `subject_id` ='$sname'");
                //$subject_name=$this->db->get_where('subject' , array('subject_id' => $sname))->result->array();

            }

            //$page_data['class_name']=$this->db->Crud_model->get_class_name($param1);
            //print_r($page_data['class_name']);exit();
            $page_data['class_id']=$param1;
            $page_data['page_name'] = 'test_class_routine';
            $page_data['page_title'] = get_phrase('manage_class_routine');
            $this->load->view('backend/index', $page_data);
        }

        function test_attendance($sort_by = 'student_id', $sort_order = 'desc', $code = 0, $page = 0, $rows = 15)
        {
            if ($this->session->userdata('admin_login') != 1)
                redirect(base_url(), 'refresh');
            ...
            $this->Search_model->attendancesheet($post);
            //print_r(date(['test']));
            ...
            $data['term'] = $term;
            $data['attendance_data'] = $this->Search_model->get_attendance($term, $sort_by, $sort_order, $rows, $page);


            $data['total'] = $this->Search_model->get_attendance_count($term);

            //print_r($data['attendance_data']);

            $this->load->library('pagination');

            $config['base_url'] = site_url('admin/test_attendance/' . $sort_by . '/' . $sort_order . '/' . $code . '/');
            $config['total_rows'] = $data['total'];
            $config['per_page'] = $rows;
            $config['uri_segment'] = 7;
            $config['first_link'] = 'First';
...
//            $page_data['class_id'] = $class_id;
            ...
            $data['page_name'] = 'manage_attendance';
            $data['page_title'] = get_phrase('manage_daily_attendance');
            $this->load->view('backend/index', $data);
        }
        ...
        function search_attendance()
        {
            ...
            $data['attendace']   =   $this->db->Search_model->s_attendance();
            echo $class;exit();
            print_r($data);
...
//            $c_name   = $this->db->get_where('class' , array('name' => $class_name))->result_array();
//            foreach ($c_name as $c)
//            {
//                $class_id=$c['class_id'];
//            }
...
//            $data['class_id']    = $class_id;
            ...
            if($param1=='borrow'){
                ...
                //echo $book_id;
                $book_available= $this->db->get_where('book',array('book_id'=>$book_id))->result_array();
                foreach ($book_available as $row)
                {
                    $b_available = $row['available'];
                }
                $new_b_available = $b_available-1;
                if($b_available>0)
                {
                    $this->db->where('book_id',$book_id);
                    $this->db->update('book', array('available' => $new_b_available));

                    //$this->db->where('book_id',$book_id);
                    $this->db->insert('book_borrow',array('book_id' => $book_id,'student_id' => $student_id, 'borrow_date' => $borrow_date, 'return_date' => $return_date));
                    //echo $b_available;
                    //print_r($book_available);
                    $this->session->set_flashdata('flash_message', get_phrase('data_added_successfully'));
                    redirect(base_url() . 'index.php?admin/book', 'refresh');
                }
            }
            ...
            $page_data['book_borrow'] = $this->db->get('book_borrow')->result_array();
            //$page_data['class_id']   = $this->db->get_where('subject' , array('class_id' => $param1))->result_array();
            //$page_data['subjects']   = $this->db->get_where('subject' , array('class_id' => $param1))->result_array();
            foreach ($page_data['books'] as $c) {
                $page_data['class_name'] = $c['name'];
            }
